...
class OverlapEngine:
    ...
    def __init__(self, config: Optional[OverlapConfig] = None, gui_callback=None):
        ...
        # "CE" and/or "PE" once a trade has been taken on that side today.
        # Survives a restart, so reopening the app does not hand back a side.
        self.sides_traded: set = set()
        ...
        self._side_warned: Dict[str, bool] = {}       # same, for the one-per-side rule
    ...
    def _track_halves(self, bars: Dict[str, Candle], bucket: int):
        """Record what each leg did on this candle — collapse, break, or neither.

        This runs on EVERY closed candle, including while a trade is open.
        It used to run only when we were flat, which meant the engine spent a
        whole trade blind to the other leg and then started from nothing the
        moment it exited. On 22-Sep the put broke its line on the very candle
        that stopped us out, at a price inside the entry band — the stop test
        read that number and the break test never saw it.

        Recording only. Nothing is armed here.
        """
        fr = self.frame
        if fr is None:
            return
        ...
        self._half_notes = {"collapse": "     ·     ".join(col_notes),
                            "breakout": "     ·     ".join(brk_notes)}

    def _try_arm(self, bars: Dict[str, Candle], bucket: int):
        """Decide whether what has been tracked adds up to a signal."""
        fr = self.frame
        if fr is None:
            return
        if self.trade_count >= self.cfg.max_trades_per_day:
        ...
        if self._past(self.cfg.last_entry_time):
            return
        notes = getattr(self, "_half_notes", {})
            ...
            for breakout_side in ("CE", "PE"):
                ...
                if self.cfg.guard_relaxed_entry:
                    ...
                    if oc is not None and oc.close > fr.borrowed_high(other):
                        ...
                        continue
                if not self._side_available(breakout_side):
                    if not self._side_warned.get(breakout_side):
                        self._side_warned[breakout_side] = True
                        self._say(f"  Relaxed entry held back — a "
                                  f"{breakout_side} trade has already been "
                                  f"taken today.")
                    continue
        ...
        for collapse_side in ("CE", "PE"):
            ...
            # The break may have come before, with, or after the collapse. What
            # matters is that the leg is STILL above its line when the pair is
            # made — `_pending_breakout` is cleared the moment it is not.
            if self.cfg.require_collapse_first and bstart < cb:
            ...
            # Both halves must be live on the candle being judged: either the
            # break just happened, or the collapse just happened and the break
            # is still standing.
            if bb != bucket and cb != bucket:
                continue

            age = abs(bstart - cb) // self.cfg.interval_seconds

            # A break that has been running a long time has carried price well
            # past the entry band, so the retest will not fill. Optional.
            if (self.cfg.max_break_age_bars > 0 and bstart < cb
                    and age > self.cfg.max_break_age_bars):
                if not self._order_warned.get(breakout_side):
                    self._order_warned[breakout_side] = True
                    self._say(f"  Not arming — {fr.leg(breakout_side).label} has "
                              f"been above its line for {age} candles, older "
                              f"than the {self.cfg.max_break_age_bars}-candle "
                              f"limit. The entry band would already be stale.")
                continue

            if not self._side_available(breakout_side):
                if not self._side_warned.get(breakout_side):
                    self._side_warned[breakout_side] = True
                    self._say(f"  Not arming — a {breakout_side} trade has "
                              f"already been taken today and only one per side "
                              f"is allowed.")
                continue

            self._arm(breakout_side, collapse_side, cb, bstart, bucket, age,
                      break_first=(bstart < cb))
            return
    ...
    def _arm(self, buy_side: str, collapse_side: str,
        ...
        if relaxed:
            ...
            when = (f"break {age_bars} candle(s) before the collapse, "
                    f"still above its line")
        else:
    ...
    def _side_available(self, side: str) -> bool:
        """Has this side already been traded today?"""
        if not self.cfg.one_trade_per_side:
            return True
        return side not in self.sides_traded
    ...
    def _on_bar_close(self, bucket: int, merge_only: bool = False):
        ...
        # Frame exists. Record what both legs did on this candle FIRST, so the
        # knowledge survives whatever happens next.
        self._track_halves(bars, bucket)

        if self.trade is not None:
            self._check_structural_stop(bars, bucket)
            if self.trade is not None:
                return
            # The stop just closed the trade on this candle. Everything the
            # other leg did is already recorded, so decide now rather than
            # starting blind on the next candle — on 22-Sep that cost an entry
            # that was sitting inside the band at the moment we exited.
            if self.signal is None:
                self._try_arm(bars, bucket)
            return
    ...
    def _enter(self, leg: Leg, price: float, sig: Signal):
        with self._lock:
            ...
            if self.trade_count >= self.cfg.max_trades_per_day:
                ...
                return
            if not self._side_available(sig.buy_side):
                self._drop_signal(f"a {sig.buy_side} trade has already been "
                                  f"taken today")
                return
            if self._past(self.cfg.last_entry_time):
    ...
    def _place_entry(self, leg: Leg, price: float, sig: Signal):
        ...
        if self.cfg.paper_mode:
            ...
            if not self._live_entry_allowed(leg, price, qty):
                return
            self._say(f"  [LIVE] Sending BUY {qty} {leg.label} at market...")
            try:
                baseline = api.position_qty(leg.sec_id)
            except Exception:
                baseline = None
            fill = api.place_order_market(leg.sec_id, leg.segment, "BUY", qty,
                                          baseline=baseline)
    ...
    def _check_orphan_position(self, leg: Leg, qty: int):
        ...
        if held:
            ...
            if over > 0:
                self._say(f"  THAT IS {over} MORE THAN THE {qty} INTENDED — "
                          f"more than one order filled. The position is "
                          f"{abs(held) / qty:.0f}x the intended size and will "
                          f"be squared off in full. Check the broker terminal.",
                          "error")
                self._emit("oversized", {"leg": leg.label, "held": abs(held),
                                         "intended": qty})
    ...
    def _adopt_position(self, leg: Leg, held: int):
        ...
        if fr is None:
            return
        side = leg.side
        # What we ACTUALLY paid, from the broker's trade book. The last traded
        # price is the price that triggered the retest, not the price the
        # orders executed at — on 23-Sep both entries were recorded at the
        # trigger price and the real fills were never known.
        entry = 0.0
        try:
            entry = api.fill_from_recent_trades(leg.sec_id, side="BUY")
        except Exception:
            entry = 0.0
        if entry > 0:
            self._say(f"  Entry price taken from the trade book: {entry:.2f}")
        else:
            entry = self._ltp_of(leg.sec_id) or 0.0
            self._say(f"  The trade book gave no fill price — recording the "
                      f"last traded price {entry:.2f} instead. Check the "
                      f"contract note; the real fill may differ.", "warning")
        if entry <= 0:
            return
        tr = Trade(
    ...
    def _finalise_trade(self, tr: Trade, exit_price: float, reason: str):
        ...
        with self._lock:
            ...
            self.trade_count += 1
            # The pending halves are NOT cleared. They are re-tested on every
            # candle and drop themselves the moment they stop being true, so
            # they reflect the legs as they are now, not stale state. Wiping
            # them here is what made the engine forget a live break the
            # instant a trade ended.
            self._order_warned.clear()
            self._side_warned.clear()
        ...
        if self.cfg.one_trade_per_side:
            left = [x for x in ("CE", "PE") if x not in self.sides_traded]
            if left and self.trade_count < self.cfg.max_trades_per_day:
                self._say(f"  {tr.side} is now used up for today. Only a "
                          f"{left[0]} entry remains.")
    ...
    def summary_payload(self) -> dict:
        ...
        return {
            ...
            "sides_traded": sorted(self.sides_traded),
            "one_per_side": self.cfg.one_trade_per_side,
            "pair": pair_txt,
            "pair_ce": pair_ce,
            "pair_pe": pair_pe,
    ...
    def load_state(self):
        try:
            ...
            self.sides_traded = set(d.get("sides_traded", []))
            if self.sides_traded and self.cfg.one_trade_per_side:
                done = " and ".join(sorted(self.sides_traded))
                self._say(f"{done} already traded today — with one trade per "
                          f"side allowed, no further {done} entry will be "
                          f"taken.")
            if self.entry_mode == "relaxed":

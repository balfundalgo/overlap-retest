...
class OverlapConfig:
    ...
    # that fired before the collapse is not counted, so no staleness limit is
    # needed — the break must be true on the candle that arms the signal.
    ...
    order_price_buffer: float = 0.10    # limit+IOC seed offset
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "order_price_buffer": self.order_price_buffer,
        }
    ...
    def from_dict(cls, d: dict) -> "OverlapConfig":
        ...
        c.order_price_buffer = float(d.get("order_price_buffer", c.order_price_buffer))
        return c

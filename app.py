...
class OverlapApp(ctk.CTk):
    ...
    def _build_settings(self, parent):
        ...
        self._switch(parent, "Collapse must be first", "require_collapse_first", False)
        self._field(parent, "Max break age (0=off)", "max_break_age_bars", "0", 70)
        ...
        self._switch(parent, "One trade per side", "one_trade_per_side", True)
    ...
    def _collect_config(self) -> OverlapConfig:
        ...
        c.require_collapse_first = bool(self.vars["require_collapse_first"].get())
        c.max_break_age_bars = _i("max_break_age_bars", 0)
        ...
        c.one_trade_per_side = bool(self.vars["one_trade_per_side"].get())
    ...
    def _handle(self, event, data):
        ...
        elif event == "oversized":
            self._log(f"OVERSIZED POSITION — {data.get('held')} of "
                      f"{data.get('leg')} against {data.get('intended')} "
                      f"intended. More than one order filled.", "error")
    ...
    def _update_stats(self, d):
        ...
        if len(self.tab_buttons) < 2:
            used = d.get("sides_traded") or []
            suffix = (f"  ({'+'.join(used)} used)"
                      if used and d.get("one_per_side") else "")
            self.stats["trades"].configure(
                text=f"{d.get('trades_done', 0)} / {d.get('trades_max', 2)}{suffix}")

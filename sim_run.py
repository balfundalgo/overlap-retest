...
def test_ordering_rule():
    ...
    # (a) break BEFORE the collapse — now accepted by default
    eng = build_engine(pair_mode="same")
    t = 1_800_000_000 - (1_800_000_000 % B)
    frame_then(eng, t, [PE_BREAKS, CE_QUIET, CE_FALLS])
    check("break before the collapse now arms", eng.signal is not None,
          "the collapse is the trigger; the break may precede it")

    # ...and is still refused when the old order is asked for
    eng_strict = build_engine(pair_mode="same", require_collapse_first=True)
    t_s = 1_800_000_000 - (1_800_000_000 % B)
    frame_then(eng_strict, t_s, [PE_BREAKS, CE_QUIET, CE_FALLS])
    check("with 'collapse must be first' it does not arm",
          eng_strict.signal is None)

    # ...and the optional age limit refuses a break that has run too long
    eng_age = build_engine(pair_mode="same", max_break_age_bars=1)
    t_a = 1_800_000_000 - (1_800_000_000 % B)
    frame_then(eng_age, t_a, [PE_BREAKS, CE_QUIET, CE_FALLS])
    check("a break older than the age limit does not arm",
          eng_age.signal is None, "2 candles old, limit 1")

    eng_age2 = build_engine(pair_mode="same", max_break_age_bars=3)
    t_a2 = 1_800_000_000 - (1_800_000_000 % B)
    frame_then(eng_age2, t_a2, [PE_BREAKS, CE_QUIET, CE_FALLS])
    check("and within the limit it does arm", eng_age2.signal is not None)
...
def test_late_start_reference():
    ...
    # A bucket safely in the past. Using today's reference bucket makes the
    # test depend on the time of day — before 09:33 it has not happened yet,
    # and a candle that has not finished is correctly refused.
    _now = int(_t.time())
    bucket = (_now - 3600) - ((_now - 3600) % iv)
    ...
    eng2 = build_engine()
    eng2.ref_bucket = lambda: bucket      # pin it to the same past bucket
...
def test_live_mode():
    ...
    sent = []
    real_mkt = dapi.place_order_market
    real_lim = real_mkt
    dapi.place_order_market = lambda *a, **k: sent.append(("BUY",) + a) or {
    ...
    try:
        ...
        dapi.place_order_market = real_mkt
    ...
    dapi.place_order_market = slow_buy
    ...
    tried = []
    dapi.place_order_market = lambda *a, **k: tried.append(a) or {
    ...
    tried2 = []
    dapi.place_order_market = lambda *a, **k: tried2.append(a) or {
    ...
    dapi.place_order_market = lambda sid, seg, side, qty, **k: (
        {"filled": True, "price": 112.0, "order_id": "O2"} if side == "BUY"
        else {"filled": False, "price": 0.0, "order_id": ""})
    ...
    e = armed(trade_mode="live", exit_retry_attempts=2)
    dapi.place_order_market = lambda sid, seg, side, qty, **k: (
        {"filled": True, "price": 112.0, "order_id": "O3"} if side == "BUY"
        else {"filled": False, "price": 0.0, "order_id": "O4"})
    ...
    e = armed(trade_mode="live")
    dapi.place_order_market = lambda *a, **k: {"filled": True, "price": 112.0,
                                                  "order_id": "O5"}
    dapi.position_qty = lambda sid: 0
    ...
    attempts = []
    dapi.place_order_market = lambda *a, **k: attempts.append(a) or {
    ...
    attempts2 = []
    dapi.place_order_market = lambda *a, **k: attempts2.append(a) or {
        "filled": False, "price": 0.0, "order_id": ""}
    try:
    ...
    dapi.place_order_market = refuse
    halts = []
    try:
        e = armed(trade_mode="live", max_entry_attempts=3,
                  entry_retry_seconds=0.0)
    ...
    dapi.place_order_market = lambda sid, seg, side, qty, **k: (
        {"filled": True, "price": 112.0, "order_id": "OK"} if side == "BUY"
        else {"filled": False, "price": 0.0, "order_id": "", "fatal": True,
              "code": "DH-905", "advice": "IP not whitelisted"})
    dapi.position_qty = lambda sid: 65
    said905 = []
    try:
    ...
    sent_after = []
    dapi.place_order_market = lambda *a, **k: sent_after.append(a) or {
        "filled": False, "price": 0.0, "order_id": ""}
    try:
        e = armed(trade_mode="live")
        e.stop_event.set()
    ...
    try:
        r = dapi.place_order_market("1", "NSE_FNO", "BUY", 65)
        ...
        check("unless it is explicitly allowed through",
              r2.get("aborted") is not True)
    finally:
...
# ═══════════════════════════════════════════════════════════════════════════
# 9j. ONE TRADE PER SIDE
# ═══════════════════════════════════════════════════════════════════════════

def test_one_trade_per_side():
    print("\n9j. Only one call trade and one put trade a day")
    B = 180
    ...
    # borrowed: CE low 103.40 / high 110.50 ; PE low 101.85 / high 109.15
    BUY_PE = {"C24450": (103.00, 104.00, 99.00, 100.00),
              "P24450": (112.00, 119.00, 111.50, 118.60)}
    BUY_CE = {"P24450": (101.00, 101.50, 96.00, 97.00),
              "C24450": (108.00, 114.00, 107.50, 113.00)}

    # ── a second trade on the SAME side is refused ──
    eng, t = framed(max_trades_per_day=4)
    feed_bar(eng, t + 2 * B, BUY_PE)
    eng._on_trade_tick("P24450", 112.00)
    check("first put trade taken", eng.trade is not None)
    if eng.trade:
        check("the side is recorded", "PE" in eng.sides_traded)
        eng._on_trade_tick("P24450", 400.00)          # target
    check("put trade closed", eng.trade is None and eng.trade_count == 1)

    eng._pending_collapse.clear(); eng._pending_breakout.clear()
    eng._break_since.clear(); eng._side_warned.clear()
    feed_bar(eng, t + 4 * B, BUY_PE)
    check("a second PUT signal does not arm", eng.signal is None,
          "one trade per side")

    # ── but the other side is still allowed ──
    feed_bar(eng, t + 5 * B, BUY_CE)
    check("a CALL signal still arms", eng.signal is not None)
    if eng.signal:
        check("and it is the call we are buying", eng.signal.buy_side == "CE")
        eng._on_trade_tick("C24450", 112.00)
        check("the call trade is taken", eng.trade is not None)
        if eng.trade:
            check("both sides are now used",
                  eng.sides_traded == {"CE", "PE"})

    # ── switched off, the same side may repeat ──
    eng2, t2 = framed(max_trades_per_day=4, one_trade_per_side=False)
    feed_bar(eng2, t2 + 2 * B, BUY_PE)
    eng2._on_trade_tick("P24450", 112.00)
    if eng2.trade:
        eng2._on_trade_tick("P24450", 400.00)
    eng2._pending_collapse.clear(); eng2._pending_breakout.clear()
    eng2._break_since.clear()
    feed_bar(eng2, t2 + 4 * B, BUY_PE)
    check("with the rule off, a second PUT signal does arm",
          eng2.signal is not None)

    # ── it survives a restart ──
    eng3, _ = framed()
    eng3.sides_traded = {"CE"}
    eng3.trade_count = 1
    eng3.save_state()
    eng4 = build_engine(pair_mode="same")
    eng4.load_state()
    check("used sides survive a restart", eng4.sides_traded == {"CE"},
          "otherwise reopening the app hands back a side")
    check("and so does the trade count", eng4.trade_count == 1)
    try:
        eng3.cfg.state_file.unlink()
    except Exception:
        pass

    # ── the entry itself is guarded, not only the arming ──
    eng5, t5 = framed(max_trades_per_day=4)
    feed_bar(eng5, t5 + 2 * B, BUY_PE)
    check("armed on the put", eng5.signal is not None)
    eng5.sides_traded.add("PE")           # as though it had been taken meanwhile
    eng5._on_trade_tick("P24450", 112.00)
    check("the entry is refused even once armed", eng5.trade is None)
    check("and the signal is dropped", eng5.signal is None)


# ═══════════════════════════════════════════════════════════════════════════
# 9k. THE 22-SEP MISS — a break seen by the stop but not by the break test
# ═══════════════════════════════════════════════════════════════════════════

def test_break_during_trade():
    print("\n9k. A break that happens while a trade is open")
    B = 180

    # The real 22-Sep prices, on a strike the test ladder carries. CE borrows the PE's 49.40 / 43.50;
    # PE borrows the CE's 48.90 / 44.30.
    MERGE = {"C24450": (44.85, 48.90, 44.30, 47.70),    # green
             "P24450": (48.35, 49.40, 43.50, 44.40)}    # red

    def framed(**kw):
        eng = build_engine(pair_mode="same", **kw)
        eng.ce_legs = [l for l in eng.ce_legs]
        t = 1_800_000_000 - (1_800_000_000 % B)
        eng.last_processed_bucket = t
        feed_bar(eng, t + B, MERGE)
        return eng, t

    eng, t = framed(max_trades_per_day=2, safety_stop_enabled=False)
    check("the pair is framed", eng.frame is not None)
    if eng.frame is None:
        return
    check("CE borrows 43.50 / 49.40",
          abs(eng.frame.ce_borrowed_low - 43.50) < 0.01
          and abs(eng.frame.ce_borrowed_high - 49.40) < 0.01)

    # collapse on the PE, then break on the CE -> long the call, as happened
    feed_bar(eng, t + 2 * B, {"P24450": (44.00, 44.20, 39.50, 40.00),
                              "C24450": (45.00, 46.00, 44.50, 45.50)})
    feed_bar(eng, t + 3 * B, {"C24450": (46.00, 51.00, 45.50, 50.25),
                              "P24450": (42.00, 43.00, 40.00, 41.00)})
    check("armed on the call", eng.signal is not None
          and eng.signal.buy_side == "CE")
    eng.book.get("C24450").last_ltp = 50.05
    eng._on_trade_tick("C24450", 50.05)
    check("in the call trade", eng.trade is not None)

    # ── the candle that stopped us out ──
    # PE closes 50.35, ABOVE its borrowed high of 48.90 — that is a break.
    # CE closes 36.10, below its borrowed low of 43.50 — that is the stop.
    eng.book.get("C24450").last_ltp = 35.50
    eng.book.get("P24450").last_ltp = 50.35
    feed_bar(eng, t + 4 * B, {"P24450": (44.00, 51.00, 43.80, 50.35),
                              "C24450": (44.00, 44.20, 35.00, 36.10)})

    check("the stop closed the call trade", eng.trade is None,
          "both charts agreed on that candle")
    check("the put's break on that same candle was recorded",
          "PE" in eng._pending_breakout,
          "the stop test read 50.35 above 48.90; the break test must too")
    check("and a signal was armed on the SAME candle", eng.signal is not None,
          "this is the entry 22-Sep missed")

    if eng.signal:
        check("it is the put we would buy", eng.signal.buy_side == "PE")
        lo, hi = eng.signal.band_low, eng.signal.band_high
        check("band is 44.90 – 52.90", abs(lo - 44.90) < 0.01 and abs(hi - 52.90) < 0.01)
        check("and the put at 50.35 is INSIDE that band", lo <= 50.35 <= hi,
              "on the day it armed six minutes later at 61.65, above the band")
        eng._on_trade_tick("P24450", 50.35)
        check("so the entry fills", eng.trade is not None)
        if eng.trade:
            check("bought the put near the line",
                  abs(eng.trade.entry_price - 50.35) < 0.01)

    # ── and the old behaviour is genuinely what changed ──
    eng2, t2 = framed(max_trades_per_day=2)
    feed_bar(eng2, t2 + 2 * B, {"P24450": (44.00, 44.20, 39.50, 40.00),
                                "C24450": (45.00, 46.00, 44.50, 45.50)})
    feed_bar(eng2, t2 + 3 * B, {"C24450": (46.00, 51.00, 45.50, 50.25),
                                "P24450": (42.00, 43.00, 40.00, 41.00)})
    eng2.book.get("C24450").last_ltp = 50.05
    eng2._on_trade_tick("C24450", 50.05)
    before = dict(eng2._pending_breakout)
    # a candle mid-trade where the put breaks but the stop does NOT fire
    feed_bar(eng2, t2 + 4 * B, {"P24450": (44.00, 51.00, 43.80, 50.35),
                                "C24450": (46.00, 47.00, 45.00, 46.50)})
    check("a break mid-trade is tracked even when no stop fires",
          "PE" in eng2._pending_breakout and "PE" not in before)
    check("but nothing is armed while the trade is open",
          eng2.signal is None and eng2.trade is not None)


# ═══════════════════════════════════════════════════════════════════════════
# 9l. THE 23-SEP DOUBLE FILL — one entry, two orders, twice the size
# ═══════════════════════════════════════════════════════════════════════════

def test_double_fill():
    print("\n9l. An entry must never place a second order on top of a fill")
    import dhan_api as dapi
    import requests as _rq

    real_post, real_get, real_del = _rq.post, _rq.get, _rq.delete
    real_pos, real_tb = dapi.position_qty, dapi.fill_from_tradebook

    # A broker whose confirmation lags the fill — every order we send executes
    # within a second, but the status poll never reports it. That is exactly
    # what the 23-Sep contract note shows.
    state = {"held": 0, "orders": 0}
    ...
    def fake_post(url, **k):
        state["orders"] += 1
        state["held"] += 65                 # every order we send fills
        return Resp({"orderId": f"O{state['orders']}", "orderStatus": "PENDING",
                     "averageTradedPrice": 0})

    def fake_get(url, **k):
        return Resp({"orderStatus": "PENDING", "averageTradedPrice": 0})

    _rq.post, _rq.get, _rq.delete = fake_post, fake_get, lambda *a, **k: Resp({})
    dapi.position_qty = lambda sid: state["held"]
    dapi.fill_from_tradebook = lambda oid: 123.70
    dapi.set_order_notifier(None)
    dapi.set_order_abort_check(None)

    try:
        # one entry -> exactly one order
        base = dapi.position_qty("73928")
        r = dapi.place_order_market("73928", "NSE_FNO", "BUY", 65, baseline=base)
        check("a market entry sends ONE order", state["orders"] == 1,
              f"{state['orders']} orders — the limit ladder sent 4")
        check("and it is recognised as filled", r.get("filled") is True, str(r)[:60])
        check("the position is the intended size", state["held"] == 65,
              f"held {state['held']}, intended 65")

        # a caller that retries must not add to it
        r2 = dapi.place_order_market("73928", "NSE_FNO", "BUY", 65, baseline=base)
        check("a retry on top of a fill sends nothing", state["orders"] == 1,
              f"{state['orders']} orders after a retry")
        check("and reports the existing fill instead",
              r2.get("already") is True, str(r2)[:60])
        check("so the position is still one lot", state["held"] == 65,
              "23-Sep ended with 130")
    finally:
        _rq.post, _rq.get, _rq.delete = real_post, real_get, real_del
        dapi.position_qty, dapi.fill_from_tradebook = real_pos, real_tb

    # ── the limit path is gone ──
    check("no limit-order path remains",
          not hasattr(dapi, "place_order_limit_ioc"),
          "entries are market orders only")

    # ── an adopted position records what we PAID, not the ticker price ──
    from config import OverlapConfig as _C
    from strategy import OverlapEngine as _E, Leg as _L
    said2 = []
    cfg2 = _C(); cfg2.trade_mode = "live"
    eng2 = _E(cfg2, gui_callback=lambda e, d: said2.append(d.get("msg", ""))
              if e == "log" else None)
    leg2 = _L("73928", 23400.0, "PE", "NIFTY 23400 PE", "NSE_FNO")
    eng2.legs = {leg2.sec_id: leg2}
    eng2.book.add(leg2.sec_id, leg2.label).last_ltp = 126.00   # the trigger price
    from strategy import Frame
    eng2.frame = Frame(bucket=0, ce=_L("73927", 23400.0, "CE", "CE", "NSE_FNO"),
                       pe=leg2, ce_candle={}, pe_candle={},
                       ce_borrowed_high=125.60, ce_borrowed_low=116.35,
                       pe_borrowed_high=124.95, pe_borrowed_low=116.70)
    real_rt = dapi.fill_from_recent_trades
    dapi.fill_from_recent_trades = lambda sid, side="BUY": 126.85   # what we paid
    try:
        eng2._adopt_position(leg2, 65)
        check("an adopted entry uses the trade-book price",
              eng2.trade is not None and abs(eng2.trade.entry_price - 126.85) < 0.01,
              f"got {eng2.trade.entry_price if eng2.trade else None}")
        check("and says where the price came from",
              any("trade book" in m for m in said2))
    finally:
        dapi.fill_from_recent_trades = real_rt

    # ── and falls back to the ticker only when the book is silent, loudly ──
    said3 = []
    eng3 = _E(cfg2, gui_callback=lambda e, d: said3.append(d.get("msg", ""))
              if e == "log" else None)
    eng3.legs = {leg2.sec_id: leg2}
    eng3.book.add(leg2.sec_id, leg2.label).last_ltp = 126.00
    eng3.frame = eng2.frame
    dapi.fill_from_recent_trades = lambda sid, side="BUY": 0.0
    try:
        eng3._adopt_position(leg2, 65)
        check("with no trade-book price it falls back to the ticker",
              eng3.trade is not None and abs(eng3.trade.entry_price - 126.00) < 0.01)
        check("and warns that the real fill may differ",
              any("contract note" in m for m in said3),
              "23-Sep recorded the trigger price with no warning at all")
    finally:
        dapi.fill_from_recent_trades = real_rt

    # ── an oversized adoption is called out ──
    from config import OverlapConfig
    from strategy import OverlapEngine, Leg
    said, events = [], []
    cfg = OverlapConfig(); cfg.trade_mode = "live"
    eng = OverlapEngine(cfg, gui_callback=lambda e, d: (
        said.append(d.get("msg", "")) if e == "log" else events.append(e)))
    leg = Leg("73928", 23400.0, "PE", "NIFTY 23400 PE", "NSE_FNO")
    eng.legs = {leg.sec_id: leg}
    eng.book.add(leg.sec_id, leg.label).last_ltp = 126.0
    dapi.position_qty = lambda sid: 130
    try:
        eng._check_orphan_position(leg, 65)
        check("an oversized position is called out",
              any("MORE THAN THE 65 INTENDED" in m for m in said),
              "silently adopting 2x size is how 23-Sep went unnoticed")
        check("and raised as an event", "oversized" in events)
    finally:
        dapi.position_qty = real_pos
...
def main():
    ...
    test_live_mode()
    test_multi_index()
    test_one_trade_per_side()
    test_break_during_trade()
    test_double_fill()

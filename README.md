...
## v3.1 — market orders only

The limit+IOC ladder is gone. Entries are a single market order.

The ladder sent up to **four orders per entry** and took **36 to 51 seconds**.
Both live failures came out of it:

- **23-Sep** — two of the four limits filled and the account carried 130
  instead of 65. The contract note shows the fills landing **one second**
  after each order went out, while the status poll never reported them.
- **22-Sep** — all three limits missed and the market fallback paid **4.4%
  slippage**, straight through the 3% cap the limits existed to enforce.

And on 23-Sep the limits were never binding anyway: every fill came in below
the limit price, at the prevailing ask — which is exactly where a market
order fills.

So the protection now sits where it can actually work: the **slippage check
made before the order is sent**, comparing the live price against the price
that triggered the retest. If it has moved more than `Max slippage %`, no
order goes out at all.

One order, one fill, about a second. `Order price buffer` is removed.

The market order still refuses to add to a position an earlier attempt has
already filled, so a caller that retries cannot double up.

---

## Fix in v3.0 — an adopted position recorded the wrong price

On 23-Sep both entries were **adopted** rather than confirmed, and an adopted
position took its entry price from the last traded price:

```python
entry = self._ltp_of(leg.sec_id) or 0.0
```

That is the price which *triggered the retest*, not the price the orders
executed at. The real fills went out at LIMIT 126.10 / 126.20 / 126.30 and
then a market order — so `126.00` in the trade log was a guess, and the P&L
computed from it was an estimate presented as a record.

An adopted position now resolves its price from the day's trade book,
quantity-weighted across every fill in that instrument:

```
Entry price taken from the trade book: 126.85
```

If the book has nothing to say, the ticker price is still used — but it says
so, instead of quietly passing a guess off as a fill:

```
The trade book gave no fill price — recording the last traded price 126.00
instead. Check the contract note; the real fill may differ.
```

---

## Fix in v2.9 — one entry could place two orders

On 23-Sep both trades were **double-sized: 130 instead of 65**.

```
10:12:02  LIMIT@126.1  attempt 1/3
10:12:12  LIMIT@126.2  attempt 2/3
10:12:21  LIMIT@126.3  attempt 3/3
10:12:28  FALLBACK -> MARKET
10:12:38  [MARKET ORDER] BUY 65 placed but no fill was confirmed
10:12:38  WARNING — the broker shows 130 of NIFTY 23400 PE
```

A limit order filled *after* we had stopped polling for its confirmation, and
the market fallback then filled as well. The retry loop treated "no
confirmation yet" as "did not fill" and sent another order on top.

The orphan-adoption logic caught the extra quantity and squared it off, so
nothing was left open — but the account had carried twice the intended risk,
and with ₹23,270 available a second double fill came close to exhausting it.

**The broker is now asked before every retry and before the market fallback.**
If an earlier attempt has already filled, nothing further is sent:

```
[ORDER] An earlier attempt had already filled 65 — not sending another.
        Treating it as done.
```

An unconfirmed *market* order is also resolved against the position rather
than assumed failed. And if a position is adopted that is larger than
intended, it is now said plainly instead of quietly accepted:

```
THAT IS 65 MORE THAN THE 65 INTENDED — more than one order filled.
The position is 2x the intended size and will be squared off in full.
```

The suite reproduces the 23-Sep broker behaviour and asserts one order, one
lot — while still proving that an order which genuinely does not fill is
retried as before.

---

## Fix in v2.8 — the engine no longer goes blind during a trade

On 22-Sep the put broke its line on the **very candle that stopped us out**,
at a price sitting inside the entry band. The stop test read that number:

```
10:27  STRUCTURAL STOP — PE closed 50.35 above 48.90 and CE closed 36.10 below 43.50
```

`50.35 above 48.90` is the break. But the break test never saw it, because
`_on_bar_close` checked the stop and returned without running the signal
tests while a trade was open. Six minutes later the put closed 61.65 and the
engine treated *that* as the start of the break — by which point it was 8.75
above the band, and it never came back. A fillable entry was watched and
discarded.

Two causes, both fixed:

- **Tracking now runs on every candle**, open trade or not. `_track_halves`
  records what each leg did; `_try_arm` decides whether it adds up to a
  signal. Only the second is gated on being flat.
- **The pending halves are no longer wiped when a trade closes.** They are
  re-tested every candle and drop themselves the moment they stop being true,
  so they describe the legs as they are — wiping them was what made the engine
  start from nothing at the exact moment it most needed to know.

And when a stop closes a trade, the signal is now evaluated **on that same
candle** rather than waiting for the next one.

The 22-Sep sequence is reproduced in the suite: it arms on the stop candle,
the band is 44.90–52.90, and the put at 50.35 fills.

---

## v2.7 — one trade per side

Once a **call** has been bought, no second call that day. The same for puts.
With two trades allowed, a day can hold at most one of each side.

`One trade per side` in the settings turns it off, and the same side may then
repeat.

It is checked in three places, because catching it late is not the same as
catching it early:

- **At arming** — a signal on a spent side is never armed, so no time is
  wasted waiting for a retest that could not be taken
- **At entry** — checked again the moment before the order, in case the side
  was used between arming and filling
- **Across restarts** — the used sides are written to the daily state file, so
  reopening the application does not hand a side back

The log says what is left after each trade:

```
CLOSED #1 NIFTY 24450 PE — TARGET. 112.20 → 129.05 (Rs.+1,094)
  PE is now used up for today. Only a CE entry remains.
```

And the top strip shows it: `TRADES 1 / 2 (PE used)`.

Note that **Max trades / day is per index**, so three indices at two each is
six trades — and with this rule, at most one call and one put on each.

...
def place_order_limit_ioc(security_id, segment, side, qty, price,
    ...
    """Limit+IOC retry engine with a market fallback. Same design as RA17."""
    fatal_seen = None
    for att in range(max_retries):
        if _aborted():
            _notify("[ORDER] Stopped — no further order sent.", "warning")
            return {"filled": False, "price": 0.0, "order_id": "",
                    "aborted": True}
        ORDER_GATE.wait()
        adj_price = round(price + price_buffer * (1 if side == "BUY" else -1), 2)
        ...
        try:
            ...
            if r.status_code == 200:
                        ...
                        try:
                            ...
                            if pr.status_code == 200:
                                ...
                                if ps == "TRADED":
                                    ...
                                    if pp > 0:
                                        log.info(f"  [FILLED] {side} qty={qty} "
                                                 f"@ Rs.{pp:.2f}")
                                        return {"filled": True, "price": pp,
                                                "order_id": oid}
                                if ps in ("REJECTED", "CANCELLED"):
                                    break
                        except Exception:
                            pass
                    try:
                        requests.delete(f"{BASE_URL}/orders/{oid}",
                                        headers=_hdrs(), timeout=10)
                    except Exception:
                        pass
            else:
                ...
                fatal_seen = None
                _notify(f"[ORDER] HTTP {r.status_code}: {r.text[:250]}")
        except Exception as e:
            _notify(f"[ORDER] {type(e).__name__}: {e}")
        price_buffer += 0.10
        time.sleep(0.5)

    if fatal_seen:
        return {"filled": False, "price": 0.0, "order_id": "",
                "fatal": True, "code": fatal_seen[0], "advice": fatal_seen[1]}
    if _aborted():
        _notify("[ORDER] Stopped — the market fallback was not sent.", "warning")
...
def place_order_market(security_id, segment, side, qty, allow_when_stopped=False) -> dict:
